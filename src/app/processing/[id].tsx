...
export default function ProcessingScreen() {
  ...
  if (!material) {
    return (
      <Screen maxWidth={560} padded={false}>
        <AppHeader onBack={() => goBackOrReplace('/(tabs)/library')} title="만드는 중" />
        <ErrorState
          description="이 기기에서 자료를 찾을 수 없어요. 홈에서 다시 열어 주세요."
  ...
  const pipelineSteps = isLink
    ? (['영상 읽기', '대본', '마인드팩'] as const)
    : (['원본 저장', '대본', '마인드팩'] as const);
  ...
  const presentation = isReady
    ? {
        ...
        description: readyResultLabels.length
          ? `${readyResultLabels.join(' / ')}까지 준비됐어요. 지금 열어 보세요.`
          : '마인드팩이 준비됐어요. 지금 열어 보세요.',
        heading: '마인드팩이 준비됐어요',
      }
    : isBlocked
      ? {
          badge: planLimited ? '분량 초과' : connectionBlocked ? '연결 대기' : '확인 필요',
          badgeTone: connectionBlocked ? ('warning' as const) : ('negative' as const),
          description: planLimited
      ...
      >
          ...
          <AnimatedReveal delay={45}>
            <Card style={styles.noticeCard} variant="soft">
              ...
              <View style={styles.flex}>
                <AppText variant="itemTitle">데모로 둘러보는 중이에요</AppText>
        ...
        {isBlocked ? (
          <ErrorState
            ...
            retryLabel={planLimited ? '구독 보기' : connectionBlocked ? '다시 시도' : '다시 만들기'}
          ...
          <AnimatedReveal delay={135}>
            <Card style={styles.stepsCard} variant="soft">
              ...
              <View style={styles.caption}>
                ...
                <AppText style={styles.flex} tone="muted" variant="meta">
                  {isReady
                    ? '모든 단계가 끝났어요. 바로 열 수 있어요.'
                    : '이 화면을 나가도 앱이 열려 있으면 계속 만들어요.'}
                </AppText>
              </View>
            </Card>
          </AnimatedReveal>
          ...
          <AnimatedReveal delay={180}>
            <Card style={styles.previewCard}>
              <View style={styles.previewHead}>
                <AppText variant="itemTitle">요약</AppText>
