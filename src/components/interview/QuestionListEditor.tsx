...
import { ANSWER_DURATION_OPTIONS, MAX_CUSTOM_QUESTIONS } from '@/features/interview/custom';
  ...
  return (
    <View style={styles.list}>
      {questions.map((question, index) => (
        <Card key={question.id} style={[styles.card, question.kind === 'follow_up' ? styles.followUp : null]}>
          <View style={styles.head}>
            <AppText tone="muted" variant="badge">{`${index + 1}번 질문`}</AppText>
            {question.kind ? <StatusBadge label={KIND_LABEL[question.kind] ?? '질문'} tone={question.kind === 'resume' || question.kind === 'follow_up' ? 'brand' : 'neutral'} /> : null}
            <View style={styles.spacer} />
            <IconButton
              disabled={disabled || questions.length <= 1}
              icon={Trash2}
              label={`${index + 1}번 질문 지우기`}
          ...
          <TextArea
            ...
            placeholder="예: 가장 도전적이었던 경험을 말해 주세요."
          ...
          <View accessibilityLabel="답변 시간" accessibilityRole="radiogroup" style={styles.chips}>
      ...
      {questions.length < MAX_CUSTOM_QUESTIONS ? (
        ...
        <AppText tone="muted" variant="meta">{`질문은 ${MAX_CUSTOM_QUESTIONS}개까지 만들 수 있어요.`}</AppText>
      )}
    </View>
  );
}
